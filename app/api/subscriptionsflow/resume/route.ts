...
// Handle POST requests for resuming subscriptions
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    // Get request body
    const body = (await request.json()) as ResumeRequestBody;
    const { subscriptionId, resumeOption, date } = body;

    if (!subscriptionId || !resumeOption) {
      return NextResponse.json(
        {
          success: false,
          error: "Subscription ID and resume option are required",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const apiEndpoint = `/api/v1/subscriptions/${subscriptionId}/resume`;
      ...
      console.error("API error resuming subscription:", apiError);
      ...
      if (axios.isAxiosError(apiError) && apiError.response) {
        ...
        if (statusCode === 405) {
          try {
            const alternativeEndpoint = `/api/v1/subscription/${subscriptionId}/resume`;
            if (apiError.config?.url?.includes(alternativeEndpoint)) {
              // If we already tried this format, try another
              const secondAlternativeEndpoint = `/api/v1/subscriptions/${subscriptionId}/resume`;
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Failed to resume your subscription. Please try again later.",
      },
      { status: 500 }
    );
  }
